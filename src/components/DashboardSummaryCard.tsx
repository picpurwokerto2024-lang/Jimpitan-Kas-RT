  ...
  return (
    <div className="w-full space-y-3" id="dashboard-summary-card">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center space-x-2 min-w-0">
          ...
          {onBack ? (
            <button
              type="button"
              onClick={onBack}
              className="w-9 h-9 rounded-full bg-[#3d2215] hover:bg-[#4d2d1d] text-white border border-white/10 flex items-center justify-center shrink-0 shadow-xs cursor-pointer transition-colors"
              title="Kembali"
          ...
          <div className="relative inline-flex items-center">
            ...
            />
            <div className={`px-3.5 py-2 rounded-full flex items-center space-x-2 text-xs sm:text-sm font-bold cursor-pointer transition-all shadow-xs ${
              isPastDate 
              ...
              <span className="truncate">{formatTanggalIndo(selectedDate)}</span>
        ...
        <div>
          {isFinished ? (
            <div className="px-3.5 py-2 rounded-full bg-[#3d2215] border border-white/10 text-emerald-300 text-xs sm:text-sm font-bold flex items-center space-x-1.5 shadow-xs">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span>Selesai</span>
              <span className="w-2 h-2 rounded-full bg-emerald-400 inline-block ml-0.5" />
            </div>
          ) : (
            <div className="px-3.5 py-2 rounded-full bg-[#3d2215] border border-white/10 text-amber-200 text-xs sm:text-sm font-semibold flex items-center space-x-1.5 shadow-xs">
              <Clock className="w-4 h-4 text-amber-300 shrink-0" />
              <span>{isPastDate ? 'Data Tanggal Lalu' : 'Sedang Berjalan'}</span>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse inline-block ml-0.5" />
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`w-full rounded-3xl ${theme.cardGradient} text-white border ${theme.cardBorder} shadow-lg p-4 sm:p-5 space-y-4 transition-all duration-300`}>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-3.5 min-w-0">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white/20 border border-white/20 flex items-center justify-center text-white flex-shrink-0 shadow-inner">
          ...
          <div className="flex items-center space-x-3 border-l border-white/15 pl-3 sm:pl-5 flex-shrink-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-amber-500/20 border border-amber-400/30 flex items-center justify-center text-amber-300 flex-shrink-0">
        ...
        <div className="flex items-center justify-between gap-3 pt-1">
          <div className="flex items-center space-x-2.5 min-w-0">
            <div className="w-8 h-8 rounded-full bg-white/20 border border-white/20 flex items-center justify-center text-white flex-shrink-0">
          ...
          <button
            ...
            className="px-4 sm:px-5 py-2 rounded-2xl bg-amber-600 hover:bg-amber-500 active:bg-amber-700 text-white font-extrabold text-xs sm:text-sm shadow-md transition-all flex items-center space-x-1.5 flex-shrink-0 cursor-pointer"
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-0.5">
        ...
        <div 
          ...
          className="bg-emerald-50/90 hover:bg-emerald-100/90 border border-emerald-200/80 rounded-2xl p-2.5 sm:p-3.5 flex items-center justify-between transition-all shadow-xs cursor-pointer group"
          id="card-stat-sudah-setor"
        >
          <div className="flex items-center space-x-2 sm:space-x-2.5 min-w-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center flex-shrink-0 shadow-xs">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] sm:text-xs font-bold text-emerald-700 truncate leading-tight">
                Sudah Setor
              </div>
            </div>
          </div>
          <ChevronRight className="w-4 h-4 text-emerald-400 group-hover:text-emerald-600 transition-colors shrink-0 ml-1" />
        </div>

        {/* Card 2: Belum Setor (Red/Pink) */}
        <div 
          onClick={() => onCardClick && onCardClick('belum')}
          className="bg-rose-50/90 hover:bg-rose-100/90 border border-rose-200/80 rounded-2xl p-2.5 sm:p-3.5 flex items-center justify-between transition-all shadow-xs cursor-pointer group"
          id="card-stat-belum-setor"
        >
          <div className="flex items-center space-x-2 sm:space-x-2.5 min-w-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-rose-500 text-white flex items-center justify-center flex-shrink-0 shadow-xs">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] sm:text-xs font-bold text-rose-700 truncate leading-tight">
                Belum Setor
              </div>
            </div>
          </div>
          <ChevronRight className="w-4 h-4 text-rose-400 group-hover:text-rose-600 transition-colors shrink-0 ml-1" />
        </div>

        {/* Card 3: Total Tagihan (Blue/Sky) */}
        <div 
          onClick={() => onCardClick && onCardClick('total')}
          className="bg-sky-50/90 hover:bg-sky-100/90 border border-sky-200/80 rounded-2xl p-2.5 sm:p-3.5 flex items-center justify-between transition-all shadow-xs cursor-pointer group"
          id="card-stat-total-tagihan"
        >
          <div className="flex items-center space-x-2 sm:space-x-2.5 min-w-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-sky-600 text-white flex items-center justify-center flex-shrink-0 shadow-xs">
              <FileText className="w-4 h-4 sm:w-4.5 sm:h-4.5 stroke-[2.5]" />
            </div>
            <div className="min-w-0">
              <div className="text-base sm:text-xl font-black text-sky-950 leading-tight">
                {totalRumah}
              </div>
              <div className="text-[10px] sm:text-xs font-bold text-sky-700 truncate leading-tight">
                Total Tagihan
              </div>
            </div>
          </div>
          <ChevronRight className="w-4 h-4 text-sky-400 group-hover:text-sky-600 transition-colors shrink-0 ml-1" />
        </div>
      </div>
